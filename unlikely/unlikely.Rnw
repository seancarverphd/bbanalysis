...
happened, but someday may. 
With a model, we can compute probabilities of
...
loaded with the number of outs unspecified?''  
To resolve this ambiguity we will always use the notation for base
occupancy together with the notation for outs. 
So \textbf{123} will always mean ``bases loaded with \emph{no} outs.''
