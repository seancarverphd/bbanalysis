...
that can make up a half-inning in a game of baseball---all of which
remain possible following the rules of the game.
Our extensive database of past played half-innings in the
major-leagues contains \Sexpr{num.distinct.half.innings} distinct
sequences.
That said, an infinitude of remaining possibilities stands
unprecedented, and for as long the game of baseball persists, whatever
infinitude that must remain will continue to tantalize us.
...
lowest probability: i.e.~which half-inning is \emph{the most unlikely
  half-inning seen in major league baseball history}?  How long can we
expect to wait to see this same sequence reappear?  As the score gets
larger, half-innings become less likely, so there does not exist a
\emph{most unlikely possible half-inning}, nor a most unlikely
half-inning that has \emph{never} been played.  However, there must
exist one (or more) \emph{most-likely-possible} half-inning(s), as
well as one (or more) that remains
\emph{most-likely-among-the-so-far-unprecedented}-half-innings.  How
long from now can we expect to wait to first see each of these
distinguished half-innings?  Though the \emph{most likely possible}
half-inning should repeat frequently enough to allow fans a correct
guess, we will nevertheless, still reveal the answer, and show that we
will not have to wait very long to see it reappear in the baseball
records.  Presumably, we would have to wait a while to see a
particular so-far-unprecedented half-inning, even the
\emph{most-likely} among that set, discussed above, as we have already
waited the entire history of the game and have not yet seen it.
That said, it should take quite a bit less time to see
\emph{any one of these unprecedented half-innings}?  How long?

With a model, we can, and will, reveal the answers to all of these
questions.  
Getting these answers came with substantial help from a computer and a
large database.
Indeed, our answers have validity only to the extent that our database
stands complete.
Thus when we say ``unprecedented,'' we really mean ``not in our database of
half-innings,'' derived from the public record, including other databases. 
...
To this end, we use a well-studied and often employed model of
baseball called baseball's Markov chain model.

So what's the shortest inning that has never been played?  We'll give
this answer now, then review the rules of baseball, explain the
notation we use, introduce the model, answer the rest of our
questions, convey some fascinating statistics and reveal some
