...
\title{The most likely, most unlikely, and possible but unprecedented occurrences in major league baseball history}
...
Baseball involves an infinity of possibilities---things that have happened before, frequently or infrequently, and things that have \emph{never} happened, yet, but someday may.
With a mathematical description of what we expect from data, in other words, \emph{a model}, we can precisely estimate probabilities of occurrences happening in the future---occurrences that may have
already happened in the past (be they commonplace or rare), and those that have not yet transpired until, or at least were never recorded up to now.

Baseball is played in epochs called innings and half-innings. 
We can characterize a half-inning in baseball by the number of plays
or `at-bats' before three outs advance the game to the next
half-inning, or the end of the game. 

With no cap on score, or number of plays, there are infinitely many
possible half-innings. 
Many, of course, have, in fact, been played before, and recorded, sometime in the
long history of the major leagues. 
That said, an infinitude of remaining possibilities have not ever been
played.
...
If this half-inning has never occurred, how long from now can we
expect to wait to see it happen? 
Of the half-innings that \emph{have} occurred, which one has the
lowest probability: i.e.~which half-inning is \emph{the most unlikely
  half-inning in baseball history}? 
With a model, we can answer these questions---with substantial help
from a powerful computer and a large database. 
We provide a link to our code below and reassure the interested reader
that most modern laptops are powerful enough to handle our
calculations, though they require user patience. 
Our calculations use a well-studied and often used model of baseball
to answer our questions: baseball's standard Markov chain model.

So what's the shortest inning that has never been played? 
We'll give this answer now, then review the rules of baseball, explain
the notation we use, introduce the model, answer our questions, convey
some fascinating statistics and reveal some compelling visualizations.

The shortest half-innings in baseball all have three plays---you need
at least three `at-bats' to get three outs. 
These half-innings (and all other half-innings, for that matter) start
with bases empty, and no outs \textbf{0}. 
While all the individual plays listed here have occurred before, the
following exact sequence of plays never has. 
We call this half-inning the ``white whale:'' first a triple,
\textbf{0:3}, second, a double with the runner on third not advancing,
\textbf{0:3:23}, and third, a triple-play (three outs),
\textbf{0:3:23:XXX}. 
That minimally short inning has never been seen in the major leagues,
or at least cannot be found in the vast Retrosheet event data set,
covering 1930 to 2018.
...
The most unlikely games in baseball's history tend to also be the highest scoring ones. 
If you ask ``what game from baseball's history would have the smallest probability that the exact same sequence of plays will reappear'' then that game (for the major leagues) would almost certainly be the one on June 28, 1871 (the first season of major league baseball) between [TRO] and [PH1] where 82 points were score.  The final score of that game was 33-49.  Only 53 points were scored in each of three next highest scoring games, all within a year of the highest scoring game.  The more plays in the sequence, the more unlikely that all of them will repeat in the future in that same sequence.

<<load,eval=T,echo=F,cache=F,message=F>>=
library(RMySQL)
library(ggplot2)
