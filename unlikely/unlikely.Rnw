...
Because a baseball diamond has three bases, there are $2^3$
possibilities for which bases have runners at the start of a play.
These 8 possibilities are denoted 
...
For a different situation, use the same notation for bases, but append
an \textbf{X} or \textbf{XX}, respectively, for one or two previous
outs.
Finally, use the notation \textbf{XXX}, without indicating which bases have
