...
words, with a \emph{model}, we can precisely estimate probabilities of
...
those that have not yet ever transpired.
...
Many, of course, have been played before, sometime in the nearly
150-year history of the major leagues.
That said, an infinitude of remaining possibilities have not ever been
played.
And no matter how long the game is played that must always remain the case.
