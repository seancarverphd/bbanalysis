...
happened before, frequently or infrequently, and things that have
\emph{never} happened, yet, but someday may.
With a mathematical description of what we expect from data, in other
words, \emph{a model}, we can precisely estimate probabilities of
...
those that have not yet transpired, or, at least, not yet recorded.
(Our records of what has happened in the past, though extensive,
remain incomplete.)
...
Many, of course, have, in fact, been played (and recorded) before,
sometime in the long history of the major leagues.  That said, an
